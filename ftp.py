...
class FTPserverThread(threading.Thread):
    ...
    def LIST(self,cmd):
        ...
        for t in os.listdir(directory):
            k=self.toListItem(os.path.join(directory,t))
...
class HTTPserverThread(threading.Thread):
    def __init__(self, (conn, addr)):
        ...
        threading.Thread.__init__(self)
    def run(self):
        print "process req from ", self.addr
        req = self.conn.recv(4096)
        print "http req %s" % req
        self.conn.send("hello")
        self.conn.close()
class HTTPserver(threading.Thread):
    def __init__(self):
        self.sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.sock.bind((local_ip, http_port))
        threading.Thread.__init__(self)
    def run(self):
        self.sock.listen(5)
        while True:
...
if __name__=='__main__':
    ...
    ftp.daemon=True
    ftp.start()
    print 'ftp On', local_ip, ':', ftp_port
    http = HTTPserver()
    http.daemon = True
    http.start()
    print 'http on', local_ip, ':', http_port
